...
离线 Whisper 不按分钟计费，主要成本是本机 CPU/GPU 时间。15 分钟音频按 32kbps MP3 压缩后只用于本机转写缓存，不需要上传语音。

## 当前边界

- 第一版只做对白检索，不做人名、声纹、人脸、物体或场景检索。
- 语音转写和语义向量默认本地处理；只有点击“智能搜索”且配置了国内大模型 Key 时，候选字幕片段会发送给大模型用于重排和总结。
- 本地处理 + 国内大模型架构见 [docs/hybrid-local-domestic-llm.md](docs/hybrid-local-domestic-llm.md)。
- 入库后请保持素材路径不变，否则预览会找不到原视频。
- 大批量素材建议先用 2 小时样本验证准确率，再跑 6-10 小时整天素材。
- 几百 TB 素材不要长期依赖 SQLite 单机索引，扩展架构见 [docs/architecture.md](docs/architecture.md)。
